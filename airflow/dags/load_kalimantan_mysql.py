...
from app.database.mysql.loaders import create_etl_run, finish_etl_run
from app.pipelines.mysql.load import DATABASE, load_province_data
...
@dag(
    dag_id="load_kalimantan_mysql",
    description="Memuat weather, hotspot, air quality, dan forecast provinsi Kalimantan ke MySQL.",
    schedule="30 10 * * *",
    start_date=pendulum.datetime(2026, 9, 22, tz=JAKARTA),
    catchup=False,
    max_active_runs=1,
    default_args={"retries": 2, "retry_delay": timedelta(minutes=15)},
    tags=["karhutla", "mysql", "kalimantan"],
)
def load_kalimantan_mysql():
    @task
        ...
        try:
            with connection() as database_connection:
                ...
                etl_run_id = create_etl_run(
                    database_connection,
                    pipeline_name="load_kalimantan_mysql",
            ...
            with connection() as database_connection:
                database_connection.select_db(DATABASE)
                finish_etl_run(database_connection, etl_run_id, "success", utc_now(), result)
            logger.info("Pipeline MySQL provinsi selesai: %s", result)
            return result
            ...
            if etl_run_id is not None:
                try:
                    ...
                    logger.exception("Gagal mencatat kegagalan ETL MySQL %s", province_id)
            raise
